...
public class StartCountdown extends BukkitRunnable {
    ...
    public StartCountdown(Match match) {
        this.match = match;
        this.match.setState(Match.MatchState.PREPARATION);

        for (Player player : this.match.getPlayers()) {
            player.setGameMode(GameMode.ADVENTURE);
        }

        count = Skywars.getInstanceConfig().getInt("game.startCountdown");
        if (count <= 0) {
            count = 10;
            Skywars.getInstance().getLogger().warning("Invalid start countdown, using default of 10 seconds.");
        }

        broadcastMessage = Skywars.getInstanceConfig().getString("messages.startCountdown.chat");
        broadcastActionBar = Skywars.getInstanceConfig().getBoolean("messages.startCountdown.actionBar");
        broadcastTitle = Skywars.getInstanceConfig().getIntegerList("messages.startCountdown.title");
